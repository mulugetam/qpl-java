...
JNIEXPORT jint JNICALL Java_com_intel_qpl_QPLJNI_execute(
    ...
    jobject input_buf, jint inputStart, jint inputSize, jbyteArray outputArr,
  ...
  if (outputArr != nullptr) {
    p_output = reinterpret_cast<uint8_t *>(
        env->GetByteArrayElements(outputArr, &is_copy_dest));
  } else {
    p_output =
  ...
  job->next_in_ptr = p_input + inputStart;
  ...
  int tmpRetry = rt;
  qpl_status status = QPL_STS_QUEUES_ARE_BUSY_ERR;
  ...
  if (outputArr != nullptr) {
    env->ReleaseByteArrayElements(outputArr,
